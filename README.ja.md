...
- 🛡️ **ACL 権限システム** — ロールベースのアクセス制御、きめ細やかな権限（read/edit/delete/create × グローバル/コレクション/エントリー/自分のみ）；ロールが権限を定義、グループがデフォルトロールを割り当て、ユーザーがオーバーライド可能
...
- 📊 **アクティビティログ** — CRUD + コメント操作の自動ログ、`/activity`
...
### ✅ 完了
- [x] ACL 権限システム（カスタムグループ + きめ細やかな read/edit/delete/create 権限、グローバル/コレクション/エントリー/自分のみスコープ対応）
...
- [x] ACL 統一リファクタリング（ロール + グループ + ユーザー、きめ細やかなアクション×スコープ権限）
