...
| 儀表板 | 條目編輯器 | 知識圖譜 |
|:-:|:-:|:-:|
| ![儀表板](./docs/screenshots/dashboard.png) | ![條目編輯器](./docs/screenshots/entry-editor.png) | ![知識圖譜](./docs/screenshots/knowledge-graph.png) |

| 搜尋 (⌘K) | Ask AI (RAG) | BPMN 流程設計器 |
|:-:|:-:|:-:|
| ![搜尋](./docs/screenshots/search.png) | ![Ask AI](./docs/screenshots/ask-ai.png) | ![BPMN](./docs/screenshots/bpmn-editor.png) |
...
- 🔒 **註冊流程** — 可設定管理員審核 + 電子郵件驗證；未驗證/未核准/已拒絕帳號有專屬登入錯誤訊息
- 🛡️ **ACL 權限系統** — 以群組為基礎的集合存取控制；每個集合可限制特定群組，角色分為 admin/editor/viewer；內建群組：「Everyone」（含匿名訪客）和「Users」（所有已註冊使用者）；有效角色取使用者所有群組中最高權限；`User.isAdmin` 可繞過所有 ACL
- 🔐 **功能權限** — 群組可設定布林開關：canCreateCollections、canUseRag、canExport、canManageWebhooks；未授權時 UI 元素隱藏且 API 回傳 403
- 👤 **使用者管理** — 管理員可刪除使用者，支援條目轉移（指派給其他使用者）或連帶刪除；顯示條目與留言數量
- 📧 **SMTP 郵件** — Gmail / 自訂 SMTP，密碼重設流程、通知郵件發送
- ⏱️ **忘記密碼頻率限制** — 每封郵件每 15 分鐘最多 3 次請求
- 🔔 **通知系統** — 應用內通知鈴鐺，SSE 即時推送、未讀徽章、標記已讀
- 💬 **留言** — 每個條目的討論串
- 📜 **版本歷史** — 自動版本記錄，附行內差異檢視器；可比較任意兩個版本或與當前內容對比
- 📊 **活動紀錄** — 自動記錄 CRUD + 留言操作，位於 `/activity`；非管理員使用者只能看到有存取權的集合中的活動
...
- 🐾 **[OpenClaw Skill + 外掛](https://github.com/hata1234/clawkb-openclaw)** — 安裝配套 Skill，讓你的 OpenClaw Agent 在聊天中直接讀取、搜尋和寫入 ClawKB 條目

### 匯入與匯出
- 📥 **匯入** — 批量匯入 Markdown (.md)、JSON 或 CSV 檔案，拖放介面、預覽表格、重複偵測（跳過/覆寫/新建）；需選擇目標集合，僅顯示有寫入權限的集合，後端驗證寫入權限
...
### ✅ 已完成
- [x] ACL 權限系統（以群組為基礎的集合存取控制，支援 admin/editor/viewer 角色 + 每群組功能權限開關）
...
- [x] ACL 統一重構（群組 × 集合 → 角色，每群組功能權限開關）
- [x] 文件編號範本（自動產生條目編號，例如 QP-{collection}-{seq:4}）
- [x] 全域浮動 AI 聊天框（任意頁面可使用 Ask AI）
- [x] BPMN 流程設計器 + Content Tag 系統（外掛 `{{tag:value}}` 架構）
- [x] 集合層級 ACL（每集合的群組存取控制）
- [x] 功能權限（每群組開關：建立集合、RAG、匯出、Webhooks）
- [x] 使用者管理（管理員刪除，支援條目轉移或連帶刪除）
- [x] 註冊流程（管理員審核 + 電子郵件驗證）
- [x] 匯入 ACL（集合選擇器，含寫入權限驗證）
- [x] 活動紀錄 ACL 篩選（非管理員只看有存取權的集合活動）
...
本專案採用 [GNU Affero General Public License v3.0](./LICENSE) 授權 — 詳見 [LICENSE](./LICENSE) 檔案。
