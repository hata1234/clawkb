...
function DiffBlock({
  ...
  const parts = mode === "line" ? diffLines(oldText, newText) : diffWords(oldText, newText);
...
function TagsDiff({ oldTags, newTags }: { oldTags: string[]; newTags: string[] }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <div>
        {removed.map((t) => (
          <span key={`r-${t}`} style={tagStyle("var(--danger)", "rgba(248,113,113,0.12)", true)}>
            {t}
          </span>
        ))}
        {unchanged.map((t) => (
          <span key={`u-${t}`} style={tagStyle("var(--text-secondary)", "var(--surface-hover)")}>
            {t}
          </span>
        ))}
        {added.map((t) => (
          <span key={`a-${t}`} style={tagStyle("var(--success, #4ade80)", "rgba(74,222,128,0.12)")}>
            {t}
          </span>
        ))}
      </div>
  ...
  const currentRevision: Revision | null = useMemo(() => {
    ...
    return {
      ...
      author: currentEntry.author
        ? {
            id: currentEntry.author.id,
            displayName: currentEntry.author.displayName,
            avatarUrl: currentEntry.author.avatarUrl,
          }
        : null,
    };
  }, [currentEntry]);
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
            <div style={flexCenterGap6}>
              ...
              >
                {currentRevision && <option value={CURRENT_ID}>{t("currentLive")}</option>}
          ...
          {revisions.map((rev, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div
                    style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0, flex: 1, overflow: "hidden" }}
                  >
                    <span
                      style={{
                        fontSize: "0.7rem",
