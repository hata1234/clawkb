...
const MODE_LABELS: Record<string, { label: string; icon: React.ReactNode; color: string }> = {
  vector: { label: "Semantic", icon: <Sparkles style={{ width: 12, height: 12 }} />, color: "var(--type-opportunity)" },
  fulltext: { label: "Full-text", icon: <TextSearch style={{ width: 12, height: 12 }} />, color: "var(--type-reference)" },
  ilike: { label: "Keyword", icon: <Type style={{ width: 12, height: 12 }} />, color: "var(--type-report)" },
  none: { label: "No results", icon: null, color: "var(--text-dim)" },
};
  ...
  return (
    <div className="search-page">
      ...
      <div style={{ marginBottom: 32, textAlign: "center" }}>
        <p style={{ fontSize: "0.7rem", color: "var(--text-dim)", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4 }}>Knowledge Base</p>
        <h1 style={{ fontFamily: "var(--font-heading)", fontSize: "2rem", fontWeight: 400, color: "var(--text)" }}>Search</h1>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto 24px" }}>
        <div style={{ position: "relative" }}>
          ...
          <input
            ...
            placeholder="Search entries by keyword or natural language..."
      ...
      {showFilters && (
        <div className="search-filters" style={{ maxWidth: 720, margin: "0 auto 24px" }}>
          <div className="search-filters-grid">
            ...
            <div>
              <label className="search-filter-label">Tag</label>
            ...
            <div>
              <label className="search-filter-label">Collection</label>
            ...
            <div>
              <label className="search-filter-label">To</label>
              <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} style={selectStyle} />
            </div>
            <div>
              <label className="search-filter-label">Search Mode</label>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--text-muted)" }}>
            ...
            <p style={{ fontSize: "1rem", fontWeight: 500, marginBottom: 8 }}>No results found</p>
            <p style={{ fontSize: "0.85rem", color: "var(--text-dim)" }}>
              Try different keywords, or switch search mode for broader results.
            </p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {results.map((result) => (
              <SearchResultCard key={result.id} result={result} searchMode={searchMode} />
            ))}
          </div>
        )}
        ...
        {!hasSearched && !loading && (
          <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--text-muted)" }}>
            ...
            <p style={{ fontSize: "0.85rem", color: "var(--text-dim)" }}>
              Use keywords for exact matches, or ask a question for semantic search.
            </p>
            <p style={{ fontSize: "0.75rem", color: "var(--text-dim)", marginTop: 16 }}>
              Tip: Press <kbd style={{ padding: "2px 6px", borderRadius: 4, background: "var(--surface)", border: "1px solid var(--border)", fontSize: "0.7rem" }}>Cmd+K</kbd> from anywhere to quick search
            </p>
          </div>
        )}
      </div>
...
function SearchResultCard({ result, searchMode }: { result: SearchResult; searchMode: string }) {
  const modeInfo = MODE_LABELS[searchMode] || MODE_LABELS.none;
        ...
        {result.tags.length > 0 && (
          <>
            ...
            {result.tags.slice(0, 3).map((tag) => (
              <span key={tag.id} className="sr-tag">{tag.name}</span>
            ))}
