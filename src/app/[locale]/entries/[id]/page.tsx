  ...
  if (error) return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 20px", textAlign: "center" }}>
      ...
      <h2 style={{ fontFamily: "var(--font-heading)", fontSize: "1.25rem", color: "var(--text)", marginBottom: 8 }}>
        {error.status === 401 ? t('loginRequired') : error.status === 403 ? t('accessDenied') : error.status === 404 ? t('notFound') : t('error')}
      </h2>
    ...
    <div style={{ maxWidth: "48rem" }}>
          ...
          {!editing ? (
                  ...
                  <div style={{ position: "absolute", top: "100%", right: 0, marginTop: 4, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", boxShadow: "0 4px 12px rgba(0,0,0,0.3)", zIndex: 50, minWidth: 140, overflow: "hidden" }}>
                    {(["json", "csv", "markdown", "pdf"] as const).map((fmt) => (
                      <button key={fmt} onClick={() => exportEntry(fmt)} style={{ display: "block", width: "100%", textAlign: "left", padding: "8px 14px", fontSize: "0.82rem", color: "var(--text-secondary)", background: "none", border: "none", cursor: "pointer", borderBottom: fmt !== "pdf" ? "1px solid var(--border)" : "none" }}
                        ...
                        {fmt === "json" ? t('json') : fmt === "csv" ? t('csv') : fmt === "markdown" ? t('markdown') : t('pdf')}
                      </button>
                    ))}
                  </div>
                )}
            ...
            <>
              ...
              <button onClick={saveEdit} disabled={saving} style={{ ...btnBase, background: "var(--accent)", color: "var(--accent-contrast)", opacity: saving ? 0.6 : 1 }}>
                {saving ? <Loader2 style={{ width: 14, height: 14 }} /> : <Check style={{ width: 14, height: 14 }} />} {tc('save')}
              </button>
            </>
          )}
        </div>
      ...
      {showDeleteConfirm && (
        <div style={{ marginBottom: 16, padding: 16, background: "rgba(248,113,113,0.05)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: "var(--radius-lg)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setShowDeleteConfirm(false)} style={{ ...btnBase, background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text-secondary)" }}>{tc('cancel')}</button>
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: "24px", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 16, fontSize: "0.8rem", color: "var(--text-muted)", marginBottom: 16 }}>
          ...
          <button onClick={toggleFavorite} style={{ display: "flex", alignItems: "center", gap: 5, background: "none", border: "none", cursor: "pointer", padding: 0, color: entry.isFavorited ? "var(--accent)" : "var(--text-dim)", transition: "color 0.15s ease" }} title={entry.isFavorited ? t('removeFromFavorites') : t('addToFavorites')}>
            <Star style={{ width: 14, height: 14, fill: entry.isFavorited ? "var(--accent)" : "none" }} /> {entry.isFavorited ? t('favorited') : t('favorite')}
          </button>
        </div>

        {editing && (
          <input value={editUrl} onChange={(e) => setEditUrl(e.target.value)} placeholder={t('urlPlaceholder')} style={{ ...inputStyle, marginBottom: 12 }} />
        )}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {editing ? (
            <input value={editTags} onChange={(e) => setEditTags(e.target.value)} placeholder={t('tagsPlaceholder')} style={{ ...inputStyle, fontSize: "0.8rem", padding: "6px 10px" }} />
          ) : entry.tags.length > 0 ? (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {entry.tags.map((tg) => (
                <span key={tg.id} style={{ fontSize: "0.75rem", background: "var(--surface-hover)", color: "var(--text-secondary)", padding: "3px 10px", borderRadius: 999 }}>
                  {tg.name}
                </span>
              ))}
            </div>
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: "20px 24px", marginBottom: 16 }}>
        ...
        {canComment ? (
          <div style={{ marginBottom: 16 }}>
            <textarea value={commentText} onChange={(event) => setCommentText(event.target.value)} placeholder={t('addComment')} style={{ ...inputStyle, minHeight: 100, resize: "vertical" }} />
          ...
          {comments.length === 0 ? (
            ...
            <div key={comment.id} style={{ background: "var(--background)", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", padding: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: "0.82rem", color: "var(--text)" }}>{comment.author?.displayName || tc('unknown')}</span>
      ...
      {showPdfPasswordDialog && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }} onClick={() => { setShowPdfPasswordDialog(false); setPdfPassword(""); }}>
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: 24, width: 360, maxWidth: "90vw", boxShadow: "0 8px 32px rgba(0,0,0,0.4)" }} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ fontFamily: "var(--font-heading)", fontSize: "1.1rem", fontWeight: 400, color: "var(--text)", marginBottom: 4 }}>{t('exportAsPdf')}</h3>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
              <button onClick={() => { setShowPdfPasswordDialog(false); setPdfPassword(""); }} style={{ ...btnBase, background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text-secondary)" }}>{tc('cancel')}</button>
