  ...
  return (
    <div style={{ maxWidth: 460, margin: "0 auto", paddingTop: 48 }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontFamily: "var(--font-heading)", fontSize: "2rem", fontWeight: 400, marginBottom: 8 }}>{t('title')}</h1>
      ...
      <form onSubmit={handleSubmit} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
        <input value={form.username} onChange={(e) => setForm((current) => ({ ...current, username: e.target.value }))} placeholder={t('usernamePlaceholder')} style={inputStyle} required />
        <input value={form.displayName} onChange={(e) => setForm((current) => ({ ...current, displayName: e.target.value }))} placeholder={t('displayNamePlaceholder')} style={inputStyle} />
        <input type="email" value={form.email} onChange={(e) => setForm((current) => ({ ...current, email: e.target.value }))} placeholder={t('emailPlaceholder')} style={inputStyle} required />
        <input type="password" value={form.password} onChange={(e) => setForm((current) => ({ ...current, password: e.target.value }))} placeholder={t('passwordPlaceholder')} style={inputStyle} required />
      ...
      <p style={{ marginTop: 16, color: "var(--text-secondary)", fontSize: "0.85rem" }}>
        {t('alreadyHaveAccount')} <Link href="/login" style={{ color: "var(--accent)" }}>{t('signIn')}</Link>
      </p>
    </div>
  );
}
