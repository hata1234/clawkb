    ...
    <div style={{ display: "grid", gap: 20 }}>
      <div style={card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
          <input value={form.id} onChange={(event) => setForm((current) => ({ ...current, id: event.target.value }))} placeholder={t('pluginIdPlaceholder')} style={inputStyle} />
          <input value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} placeholder={t('pluginNamePlaceholder')} style={inputStyle} />
          <input value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} placeholder={tc('description')} style={inputStyle} />
          <input value={form.hooks} onChange={(event) => setForm((current) => ({ ...current, hooks: event.target.value }))} placeholder={t('hooksPlaceholder')} style={inputStyle} />
        </div>
            ...
            <div key={plugin.id} style={{ background: "var(--background)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 10 }}>
                <div>
                  ...
                  <div style={{ color: "var(--text-secondary)", fontSize: "0.82rem", marginTop: 4 }}>{plugin.id} · v{plugin.version}{plugin.builtIn ? ` · ${t('builtIn')}` : ""}</div>
                </div>
