  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: 24 }}>
      <div style={{ display: "grid", gap: 14 }}>
        {(["allowRegistration", "requireAdminApproval", "requireEmailVerification", "allowAgentRegistration"] as const).map((key) => (
      ...
      {message ? <div style={{ marginTop: 14, color: message === t('authSettingsSaved') ? "var(--accent)" : "var(--danger)", fontSize: "0.85rem" }}>{message}</div> : null}
    </div>
  );
}
