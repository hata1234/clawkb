...
function parseJSON(text: string): ParsedEntry[] {
  ...
  return arr
    .map((item: Record<string, unknown>) => ({
      ...
      metadata:
        typeof item.metadata === "object" && item.metadata ? (item.metadata as Record<string, unknown>) : undefined,
    }))
...
export default function ImportClient() {
    ...
    async (fileList: File[]) => {
      ...
      for (const file of fileList) {
        try {
          ...
          setParseError(
            t("failedToParse", { name: file.name, error: err instanceof Error ? err.message : "unknown error" }),
          );
        }
      }
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 20px" }}>
      ...
      <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", marginBottom: 32 }}>{t("importDescription")}</p>
      ...
      {!results && (
        ...
        >
          ...
          <p style={{ color: "var(--text-dim)", fontSize: "0.8rem" }}>{t("supportedFormats")}</p>
          <input
            ref={fileInputRef}
            type="file"
            multiple
            ...
            >
              <div
                style={{
                  fontSize: "1.5rem",
                  fontWeight: 700,
                  color: results.errors.length > 0 ? "var(--danger)" : "var(--text-dim)",
                }}
              >
          ...
          {results.errors.length > 0 && (
            ...
            >
              {results.errors.map((e, i) => (
                <div key={i} style={{ fontSize: "0.78rem", color: "var(--danger)", marginBottom: 4 }}>
                  {e}
                </div>
              ))}
            </div>
