  ...
  const doImport = useCallback(async () => {
    ...
    for (let b = 0; b < totalBatches; b++) {
      ...
      try {
        ...
        if (!res.ok) {
          combined.errors.push(data.error || `Batch ${b + 1} failed`);
        } else {
        ...
        combined.errors.push(`Batch ${b + 1}: network error`);
      }
    ...
    setResults(combined);
    setImporting(false);
  }, [parsed, defaultTags, duplicateHandling]);
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 20px" }}>
      ...
      <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", marginBottom: 32 }}>
        Import entries from Markdown, JSON, or CSV files.
      </p>
      ...
      {parsed.length > 0 && !results && (
        <div style={{ ...sectionStyle, marginBottom: 24 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
            <div>
              <label style={labelStyle}>Duplicate Handling</label>
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. imported, review"
                style={inputStyle}
              />
            </div>
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <th style={{ padding: "10px 16px", textAlign: "left", color: "var(--text-secondary)", fontWeight: 500 }}>Title</th>
                  <th style={{ padding: "10px 16px", textAlign: "left", color: "var(--text-secondary)", fontWeight: 500 }}>Type</th>
                  <th style={{ padding: "10px 16px", textAlign: "left", color: "var(--text-secondary)", fontWeight: 500 }}>Tags</th>
                  <th style={{ padding: "10px 16px", textAlign: "left", color: "var(--text-secondary)", fontWeight: 500 }}>Content</th>
        ...
        <div style={{ ...sectionStyle, marginBottom: 24, display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ color: "var(--text)", fontSize: "0.85rem", fontWeight: 500 }}>Importing…</span>
      ...
      {results && (
        <div style={{ ...sectionStyle, marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            ...
            <h3 style={{ fontSize: "1.1rem", fontWeight: 600, color: "var(--text)", margin: 0 }}>Import Complete</h3>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: results.errors.length > 0 ? 16 : 0 }}>
            <div style={{ textAlign: "center", padding: 16, background: "var(--background)", borderRadius: "var(--radius-sm)" }}>
              ...
              <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", marginTop: 4 }}>Created</div>
            </div>
            <div style={{ textAlign: "center", padding: 16, background: "var(--background)", borderRadius: "var(--radius-sm)" }}>
              <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "var(--warning)" }}>{results.skipped + results.overwritten}</div>
              <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", marginTop: 4 }}>
                {results.overwritten > 0 ? `${results.skipped} skipped, ${results.overwritten} overwritten` : "Skipped"}
              </div>
            </div>
            <div style={{ textAlign: "center", padding: 16, background: "var(--background)", borderRadius: "var(--radius-sm)" }}>
              <div style={{ fontSize: "1.5rem", fontWeight: 700, color: results.errors.length > 0 ? "var(--danger)" : "var(--text-dim)" }}>{results.errors.length}</div>
              <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", marginTop: 4 }}>Errors</div>
            </div>
          </div>
