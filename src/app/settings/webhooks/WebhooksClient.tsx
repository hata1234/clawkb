...
function DeliveriesPanel({ webhookId }: { webhookId: number }) {
  ...
  if (deliveries.length === 0) return <p style={{ fontSize: "0.8rem", color: "var(--text-dim)" }}>No deliveries yet.</p>;
...
export default function WebhooksClient() {
  ...
  async function createWebhook() {
    ...
    if (formEvents.length === 0) { showToast("Select at least one event", false); return; }
    setCreating(true);
    try {
      const res = await fetch("/api/webhooks", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        showToast(err.error || "Failed to create webhook", false);
      }
    } finally {
      setCreating(false);
    }
  ...
  async function toggleActive(webhook: WebhookData) {
    ...
    if (res.ok) {
      showToast(webhook.active ? "Webhook disabled" : "Webhook enabled", true);
      fetchWebhooks();
    }
  }
  ...
  async function deleteWebhook(id: number) {
    ...
    if (res.ok) {
      ...
      showToast("Failed to delete webhook", false);
    }
  }
    ...
    <div>
      ...
      {revealedSecret && (
        <div style={{
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button onClick={() => { navigator.clipboard.writeText(revealedSecret); showToast("Copied", true); }} style={btnPrimary}>
      ...
      <div style={card}>
        ...
        <p style={{ fontSize: "0.8rem", color: "var(--text-secondary)", marginBottom: showForm ? 16 : 0 }}>
          Webhooks send HTTP POST requests to your URL when events occur. Payloads are signed with HMAC-SHA256.
        </p>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
              <div style={{ flex: 1 }}>
                ...
                <input value={formName} onChange={(e) => setFormName(e.target.value)} style={inputStyle} placeholder="My webhook" />
              </div>
              <div style={{ flex: 2 }}>
      ...
      {!loading && webhooks.length === 0 && !showForm && (
        <div style={{ ...card, textAlign: "center", color: "var(--text-dim)" }}>
          <p style={{ fontSize: "0.875rem" }}>No webhooks configured yet.</p>
        </div>
      )}
