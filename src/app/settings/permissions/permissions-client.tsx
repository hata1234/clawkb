  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h2 style={{ fontFamily: "var(--font-heading)", fontSize: "1.15rem", fontWeight: 400, color: "var(--text)" }}>Permission Groups</h2>
          <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginTop: 4 }}>
            Manage groups and their permissions. Users inherit permissions from all groups they belong to.
          </p>
        </div>
      ...
      {showCreateGroup && (
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: 16, marginBottom: 16, display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 200 }}>
            <label style={{ display: "block", fontSize: "0.75rem", color: "var(--text-dim)", marginBottom: 4 }}>Group Name</label>
            <input value={newGroupName} onChange={(e) => setNewGroupName(e.target.value)} placeholder="e.g. Research Team" style={inputStyle} />
          </div>
          <div style={{ flex: 2, minWidth: 200 }}>
            <label style={{ display: "block", fontSize: "0.75rem", color: "var(--text-dim)", marginBottom: 4 }}>Description</label>
            <input value={newGroupDesc} onChange={(e) => setNewGroupDesc(e.target.value)} placeholder="Optional description" style={inputStyle} />
          </div>
          <button onClick={createGroup} style={{ ...btnStyle, background: "var(--accent)", color: "var(--accent-contrast)", border: "none" }}>Create</button>
            ...
            <div key={group.id} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", overflow: "hidden" }}>
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {group.builtIn && <span style={{ fontSize: "0.65rem", background: "var(--accent-muted)", color: "var(--accent)", padding: "1px 6px", borderRadius: 999, marginLeft: 8, fontWeight: 600 }}>Built-in</span>}
                ...
                <div style={{ borderTop: "1px solid var(--border)", padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <h4 style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.06em" }}>Permissions</h4>
                    ...
                    {addPermGroup === group.id && (
                      <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
                        ...
                        <button onClick={() => addPermission(group.id)} style={{ ...btnStyle, background: "var(--accent)", color: "var(--accent-contrast)", border: "none", padding: "4px 12px", fontSize: "0.75rem" }}>Add</button>
                      </div>
                    )}
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      ...
                      {group.permissions.length === 0 && <span style={{ fontSize: "0.75rem", color: "var(--text-dim)" }}>No permissions</span>}
                    </div>
                  </div>
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <h4 style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.06em" }}>Members</h4>
                    ...
                    {addUserGroup === group.id && (
                      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                        ...
                        <button onClick={() => addUserToGroup(group.id)} style={{ ...btnStyle, background: "var(--accent)", color: "var(--accent-contrast)", border: "none", padding: "4px 12px", fontSize: "0.75rem" }}>Add</button>
                      </div>
                    )}
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      ...
                      {group.users.length === 0 && <span style={{ fontSize: "0.75rem", color: "var(--text-dim)" }}>No members</span>}
                    </div>
                  </div>
                </div>
              )}
